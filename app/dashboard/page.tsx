    ...
    <div className="container mx-auto px-4 py-8 bg-gray-50">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {Object.entries(processTypeCategories).map(([category, types]) => (
            <div key={category} className="bg-white rounded-lg shadow-md p-4">
              <h3 className="text-lg font-semibold text-navy mb-3">{category}</h3>
              <div className="space-y-2">
                {types.map(type => {
                  const count = processesByType[type]?.length || 0;
                  return (
                    <Link 
                      key={type} 
                      href={`/dashboard/${type}`}
      ...
      <div className="mb-8">
        <div className="flex justify-between items-center mb-4">
          ...
          <Link href="/processo" className="text-navy hover:text-gold">
            View All
          </Link>
        </div>
      ...
      <div className="p-2 space-y-2 max-h-[400px] overflow-y-auto">
        ...
        {processes.length > 5 && (
          <div className="text-center pt-2 pb-1">
            <Link href="/processo" className="text-sm text-navy hover:text-gold">
...
function ProcessCard({ process }: { process: any }) {
  return (
    <Link href={`/processo/${process.process_key}`}>
